...
```{r}
yellowtail<-read.csv("yellowtail.csv")
summary(yellowtail)
...
res2jags<-jags(yellowtailList,list(init1,init2),model.file="VonBertLnormSchnute.txt",
...
write("model {
 ...
 chi.square.rep<-sum(rep.sresid2[])
 p.value<-step(chi.square.obs-chi.square.rep)
}","VonBertNormalSchnute.txt")

init1<-list(tau=1,L1=500,L2=1200,K=0.6)
init2<-list(tau=0.5,L1=400,L2=1000,K=0.4)
res3jags<-jags(yellowtailList,list(init1,init2),model.file="VonBertNormalSchnute.txt",
...
",
  file="stanGrowthLN.stan")
yellowtailStan1<-stan(file="stanGrowthLN.stan",
  data=yellowtailList)
...
",
  file="stanGrowthLNSchnute.stan")

yellowtailList2=list(N=dim(yellowtail)[1],Age=yellowtail$Age,ObsL=yellowtail$ObsL,
  Age1=1,Age2=8)

yellowtailStan2<-stan(file="stanGrowthLNSchnute.stan",
...
resdug<-jags(dugong,list(init1,init2),model.file="dugong.txt",
...
write("model {
  ...
  log(sigma2) <- 2*log.sigma
  log.sigma    ~ dunif(-10, 10)
}",file="dugongNoError.txt")
resdug2<-jags(dugong,list(init1,init2),model.file="dugongNoError.txt",
...
",file="growthZhangOriginalJags.txt")
zhang1j<-jags(altbacoreList,list(init1,init2),model.file="growthZhangOriginalJags.txt",
